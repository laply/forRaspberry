...
class Control:
    ...
    def check(self):
        ...
        self.lastdata[0] = pm2p5
        self.lastdata[1] = pm10

        if self.tHCount == 5 :
            self.topic.setSendMessageTopic(0, self.pm10TopicNum, pm2p5)
            self.topic.setSendMessageTopic(0, self.pm2p5TopicNum, pm10)
            self.tHCount = 0
